...
12. **Sequential Docker restart accumulates fatal delay for grace-period-based monitors** — `container.restart()` blocks until the container is running; restarting 7 containers sequentially takes ~14s. If the SC restarts 3rd in the queue (T=4), its 5s grace expires at T=9 — but CVC doesn't start until T=16. The SC kills the relay for CVC heartbeat loss before CVC even boots. **Fix**: restart SC after all heartbeat senders (FZC/RZC) but before CVC, so SC's grace window covers the CVC boot. **Rule**: any container with a fixed-duration grace period must restart last among its peers, not in the middle of a sequential restart chain
