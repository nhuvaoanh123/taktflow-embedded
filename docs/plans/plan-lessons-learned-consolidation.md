...
**Goal:** Extract every lesson learned from 200 commits into `docs/reference/lessons-learned/` — single point of truth for human-AI co-development improvement.
...
**Source commits:** `90e6e90`, `a92ea24`, `d945fba`→`cab9030` (7 MISRA CI fix commits), `62992df`
**Lessons:**
...
- 1,536 → 0 violations in 1 day — triage strategy
- stderr redirect order matters for cppcheck

### 2. `PROCESS-ci-test-hardening.md`
**Source commits:** `5650705`→`5d94469` (8 rounds of CI fixes), `cfa0fcd`, `52e46c1`, `9f13a0d`, `375194e`, `083f9df`
**Lessons:**
...
- 99 test failures from CI/host differences — batch strategy
...
**Source commits:** `6772c91`→`b19deb3` (Phase 11-17), `b99db6e`→`0fc07e6` (SIL fixes)
**Lessons:**
...
- RTE_MAX_SIGNALS too small — silent init failure
- Plant simulator thermal model needs tuning (not just copy physics)
- ML anomaly score stuck at baseline — needs state-aware thresholds
- E-STOP event spam on reset — need proper fault clear sequence
...
- TDD for 16+ BSW modules: write test stub → define API → implement
...
- HITL comments only after discussion — never pre-written

### 7. `PROCESS-architecture-decisions.md`
**Source commits:** `c36e4b8`, `542e42d`, `eb61ea6`, `b662a35`, `bf7a9ff`→`54505d8`
**Lessons:**
- ADR format with T1-T4 tier + 4-dimension scoring
- Decision audit script catches undocumented decisions
- 7-ECU hybrid (4 physical + 3 simulated) — best of both worlds
- Custom trace-gen.py over Polarion/DOORS/Jama (ADR-013)
- File-based ALM over heavyweight tools — git is the database
...
3. Done — no commit until user approves
